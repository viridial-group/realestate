  ...
  <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 p-4">
    <div class="w-full max-w-md">
      ...
      <div class="text-center mb-8">
        ...
        <h1 class="text-3xl font-bold text-foreground mb-2">{{ t('auth.login') }}</h1>
      ...
      <Card class="shadow-lg border-0">
        <CardContent class="p-6 space-y-6">
          ...
          <div class="flex gap-3">
            <Button variant="outline" class="flex-1" size="lg" @click="handleSocialLogin('google')" :disabled="loading">
            ...
            <Button variant="outline" class="flex-1" size="lg" @click="handleSocialLogin('apple')" :disabled="loading">
              <svg class="w-5 h-5 mr-2" viewBox="0 0 24 24" fill="currentColor">
...
import { useAuth } from '@viridial/shared'
...
const handleSocialLogin = async (provider: 'google' | 'apple') => {
  ...
  try {
    ...
    toast({
      ...
      description: `Connexion avec ${provider} sera disponible prochainement`
    })
  } catch (error) {
