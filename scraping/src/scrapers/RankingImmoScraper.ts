...
export class RankingImmoScraper extends PuppeteerScraper {
  getName(): string {
    return 'RankingImmo';
  }
  ...
  async scrape(searchTerm: string, city?: string): Promise<Agency[]> {
    ...
    const searchUrl = `https://www.rankingimmo.fr/recherche?q=${encodeURIComponent(query)}`;
    ...
    try {
      ...
      const results = await page.evaluate((selectors) => {
        ...
        const containerSelectors = [
          selectors.container,
          '.agency-listing', '.agency-item', '.result',
          '[class*="agency"]', '[class*="listing"]', '.card'
        ];
        ...
        if (!resultElements || resultElements.length === 0) {
          const allLinks = document.querySelectorAll('a[href*="agence"], a[href*="agency"]');
        ...
        resultElements.forEach((element: Element) => {
          try {
            ...
            const websiteElement = element.querySelector(selectors.website || 'a[href^="http"]:not([href*="rankingimmo"])');
      ...
      logger.error(`Error scraping RankingImmo: ${error.message}`);
      if (page) {
        try {
          await page.close();
        } catch (e) {
